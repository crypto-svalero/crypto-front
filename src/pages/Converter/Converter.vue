          ...
          <b-col lg="6">
            ...
            <div class="converter__currency">
              <select id="selectCrypto">
                  <option v-for="crypto in cryptos" v-bind:key="crypto.id" :value="crypto.currentValue" :selected= "crypto.id == 1" >                    
                        {{crypto.name}}
                  </option>
              </select>
              <input id="numberCrypto" type="number" class="converter__form__input" value="0" />
            </div>
          </b-col>
          <span class="swap-icon" @click="swapColumns()">
            <i class="glyphicon glyphicon-forward"></i>
          </span>
           <button id="buttonConverte" @click="convert()">Convert</button>
          <b-col lg="6">
            <h5>Fiat</h5>
            <div class="converter__currency">
              <select id="selectFiat">
                  <option v-for="fiat in fiats" v-bind:key="fiat.id" :value="fiat.value" :selected= "fiat.id == 1" >
                        {{fiat.name}} 
                  </option>
              </select>
              <input disabled id="numberFiat" type="number" class="converter__form__input" value="0" />
            </div>
          </b-col>
        </form>
      </Widget>
...
export default {
  ...
  methods: {
    ...
    convert(){
      if(document.getElementById("numberFiat").disabled == true) {
      document.getElementById("numberFiat").value = document.getElementById("numberCrypto").value * document.getElementById("selectCrypto").value / document.getElementById("selectFiat").value;
      document.getElementById("numberFiat").value.toFixed(2);
      }else{
      document.getElementById("numberCrypto").value = document.getElementById("numberFiat").value * document.getElementById("selectFiat").value / document.getElementById("selectCrypto").value;
